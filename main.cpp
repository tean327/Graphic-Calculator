...
int CreateWindow();
int DrawFunction(Shader *pProgram, VAO* VAO);
int DrawGraph(Shader *pProgram, VAO* VAO);
int AddVertices();
float f(float x);
#pragma endregion
...
int HandleInput(std::string input)
{
	...
	if (count == input.length())
		constante = stoi(input);

	// Handle functions input
	else if (input == "sin(x)" || input == "sin")
		func = sin;
	else if (input == "cos(x)" || input == "cos")
		func = cos;
	else if (input == "tan(x)" || input == "tan")
		func = tan;
	else if (input == "asin(x)" || input == "asin")
		func = asin;
	else if (input == "acos(x)" || input == "acos")
		func = acos;
	else if (input == "atan(x)" || input == "atan")
		func = atan;
	...
	else if (input == "square" || input == "Vx")
		func = sqrt;
	else if (input == "log(x)")
		func = log;
	else if (input == "log2(x)")
		func = log2;
	else if (input == "log10(x)")
		func = log10;
	else if (input == "e^x")
		func = exp;

	// Handle consts input
	else if (input == "PI" || input == "pi" || input == "π")
		constante = PI;
...
int CreateWindow()
{
	...
	VAO VAOGraph;
	VAO VAOFunc;
	DrawGraph(&shaderProgram, &VAOGraph);
	DrawFunction(&shaderProgram, &VAOFunc);
...
int DrawFunction(Shader* pProgram, VAO* VAO)
{
	...
	VAO->UnBind();
	VBO1.UnBind();
	uniID = glGetUniformLocation(pProgram->ID, "scale");
